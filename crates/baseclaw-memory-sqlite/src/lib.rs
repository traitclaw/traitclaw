//! SQLite memory backend for the `BaseClaw` AI agent framework.
//!
...
use baseclaw_core::traits::memory::{Memory, MemoryEntry};
use baseclaw_core::types::message::{Message, MessageRole};
use baseclaw_core::Result;
...
impl SqliteMemory {
    ...
    pub fn new(path: &str) -> Result<Self> {
        let conn = Connection::open(path)
            .map_err(|e| baseclaw_core::Error::Runtime(format!("SQLite open error: {e}")))?;
...
fn init_schema(conn: &Connection) -> Result<()> {
    ...
    .map_err(|e| baseclaw_core::Error::Runtime(format!("Schema init error: {e}")))?;
...
impl Memory for SqliteMemory {
    async fn messages(&self, session_id: &str) -> Result<Vec<Message>> {
        let conn = self
            .conn
            .lock()
            .map_err(|e| baseclaw_core::Error::Runtime(format!("Lock error: {e}")))?;
        let mut stmt = conn
            .prepare("SELECT role, content, tool_call_id FROM messages WHERE session_id = ?1 ORDER BY id")
            .map_err(|e| baseclaw_core::Error::Runtime(format!("Query error: {e}")))?;
        ...
        for row in rows {
            messages
                .push(row.map_err(|e| baseclaw_core::Error::Runtime(format!("Row error: {e}")))?);
        }
        Ok(messages)
    }
    ...
    async fn append(&self, session_id: &str, message: Message) -> Result<()> {
        ...
        .map_err(|e| baseclaw_core::Error::Runtime(format!("Insert error: {e}")))?;
        Ok(())
    }
    ...
    async fn get_context(&self, session_id: &str, key: &str) -> Result<Option<Value>> {
        ...
        match result {
            ...
            Err(e) => Err(baseclaw_core::Error::Runtime(format!("Query error: {e}"))),
        }
    }
    ...
    async fn set_context(&self, session_id: &str, key: &str, value: Value) -> Result<()> {
        ...
        let json_str = serde_json::to_string(&value)
            .map_err(|e| baseclaw_core::Error::Runtime(format!("JSON error: {e}")))?;
        conn.execute(
    ...
    async fn store(&self, entry: MemoryEntry) -> Result<()> {
        ...
        let rowid: i64 = conn
            ...
            .map_err(|e| baseclaw_core::Error::Runtime(format!("Rowid error: {e}")))?;

        conn.execute(
            "INSERT OR REPLACE INTO long_term_fts (rowid, content) VALUES (?1, ?2)",
            rusqlite::params![rowid, entry.content],
        )
        .map_err(|e| baseclaw_core::Error::Runtime(format!("FTS insert error: {e}")))?;
    ...
    async fn list_sessions(&self) -> Result<Vec<String>> {
        ...
        let mut ids = Vec::new();
        for row in rows {
            ids.push(row.map_err(|e| baseclaw_core::Error::Runtime(format!("Row error: {e}")))?);
        }
        Ok(ids)
    }
    ...
    async fn delete_session(&self, session_id: &str) -> Result<()> {
        ...
        .map_err(|e| baseclaw_core::Error::Runtime(format!("Delete error: {e}")))?;
